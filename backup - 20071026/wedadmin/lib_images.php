...
function resize_image_oneparam($filename, $new_filename, $maxwidth, $maxheight){
	...
	$c_url = '/catalogue/customimages/';
	$c_path = NX_PATH.'catalogue/customimages/';

print "$c_url - $c_path<BR>\n";
	...
	if (isset($_GET["id"])){
		...
		$i = 0;
		while ($i < ($pages_count)){
			echo ' <A href="/wedadmin/lib_images.php?id='.$id.'&page='.$i.'">';
			if ($i == $page){
		...
		foreach ($files as $filename){
			...
			$imginfo = getimagesize($c_path.$filename);
			if ($imginfo){
				echo '<form action="/wedadmin/lib_images.php?id='.$id.'&resize" method="post">';
			...
			echo '</TD><TD><A href="/wedadmin/lib_images.php?id='.$id.'&delete='.base64_encode($filename).'" onClick="javascript: if (confirm(\'Удалить?\')) { return true;} else { return false;}"><img src="/wedadmin/images/b_drop.png" border="0" /></A></TD></TR>';
		}
		echo '</TABLE>';
		echo '<BR/><FORM method="post" enctype="multipart/form-data" action="/wedadmin/lib_images.php?id='.$id.'&upload"><INPUT type="file" name="picture"><BR/><BR/><INPUT type="submit" value="добавить"></FORM><BR/><BR/>';
	}
