...
## delete picture
	...
	if (isset($_GET["updatei"])){
		...
		if (count($date_a)==3){
			...
			if ($_FILES['small_picture']['name']) {
				...
				resize($new_filename, $to, $type, 200);
				unlink($new_filename);
				$small_uploaded = TRUE;
			}
		...
		if ($item = mysql_fetch_array(mysql_query("SELECT * FROM `wed_news_items` WHERE `id` = '$id'"))){
			...
			$xihna_enabled = true;	
                ...
                if ($item["small_picture"] !== ''){
                	...
                	echo '<BR/>загружено:'.$item["small_picture"].'<BR/><IMG src="/small/'.$item["small_picture"].'">';
	...
	$query = mysql_query("SELECT * FROM `wed_news_items` ORDER BY `is_favorite` DESC, `date` DESC LIMIT $from, $count");
