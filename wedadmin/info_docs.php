<?php
	...
	if(isset($_SESSION['WA_USER']) && db_open())
	{
		...
		function save_doc($id)
		{
			if(isset($_POST['pid'], $_POST['type'], $_POST['name']))
			{
				...
				if($id && ($qr = mysql_query('SELECT `type` FROM `wed_docs` WHERE `id`='.$id)) !== false && mysql_num_rows($qr))
				{
					...
					{
						case 1:
							mysql_query('UPDATE `wed_docs` SET `pid`='.$pid.', `name`="'.$name.'", `ext`="", `description`="'.$description.'" WHERE `id`='.$id);
							return;
						case 2:
							if(isset($_POST['text']) && ($f = fopen("../info/{$id}.dat", 'wb')) !== false)
							{
								fwrite($f, $_POST['text']);
								fclose($f);
								mysql_query('UPDATE `wed_docs` SET `pid`='.$pid.', `name`="'.$name.'", `ext`="html", `description`="'.$description.'" WHERE `id`='.$id);
							}
							return;
						case 3:
							if(isset($_FILES['file']['tmp_name'], $_FILES['file']['name']) && file_exists($_FILES['file']['tmp_name']))
							{
								...
								mysql_query('UPDATE `wed_docs` SET `pid`='.$pid.', `name`="'.$name.'", `ext`="'.$ext.'", `description`="'.$description.'" WHERE `id`='.$id);
							}
							else
								mysql_query('UPDATE `wed_docs` SET `pid`='.$pid.', `name`="'.$name.'", `description`="'.$description.'" WHERE `id`='.$id);
							return;
					}
				}
				else switch($type)
				{
					case 1:
						$Q="INSERT INTO `wed_docs` SET `pid`=$pid, `type`=1, `name`='$name', `ext`='', `description`='$description'";
						#print $Q;
						mysql_query($Q);
								
						return;
					case 2:
						if(mysql_query("INSERT INTO `wed_docs` SET `pid`=$pid, `type`=2, `name`='$name', `ext`='html', `description`='$description'"))
						{
							$id = mysql_insert_id();
							if(isset($_POST['text']) && ($f = fopen("../info/{$id}.dat", 'wb')) !== false)
							{
					...
					case 3:
						if(isset($_FILES['file']['tmp_name'], $_FILES['file']['name']) && file_exists($_FILES['file']['tmp_name']))
						{
							...
							if(preg_match('/\.([^\.]+)$/ims', $_FILES['file']['name'], $vals))
								if(strlen($vals[1]) < 6)
									$ext = addslashes($vals[1]);
							$Q='INSERT INTO `wed_docs` SET `pid`='.$pid.', `type`=3, `name`="'.$name.'", `ext`="'.$ext.'", `description`="'.$description.'"';	
							#print $Q;
							if(mysql_query($Q))
		...
		function edit_doc($id, $pid, $type = 0)
		{
			...
			if($id && ($qr = mysql_query('SELECT `pid`, `type`, `name`, `ext`, `description` FROM `wed_docs` WHERE `id`='.$id)) !== false && mysql_num_rows($qr))
				list($pid, $type, $name, $ext, $description) = mysql_fetch_row($qr);
...
<?php
		...
		if(($qr = mysql_query('SELECT `id`, `type`, `name`, `ext`, `description` FROM `wed_docs` WHERE `type`=1 AND `pid`='.$id)) !== false)
		{
...
?>
		<tr class="r<?=$i?>">
			...
			<td><?=$type?></td>
			<td>
				<a href="<?=$script.'?delete&amp;target='.intval($row[0]).'&amp;id='.$id?>"><img src="images/icons/b_drop.png" border="0" alt="" /></a>
			</td>
		</tr>
<?php
					$i = ($i + 1) % 2;
