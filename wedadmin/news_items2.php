...
## delete picture
	...
	if (isset($_GET['delete_picture'])&&isset($_GET['id'])){
	  ...
	  # update
		...
		$string="select small_picture from `wed_news_items2` where `id`=$id";	
		...
		$string="update `wed_news_items2` set small_picture='' where `id`='$id'";
	...
	if (isset($_GET["fav"])){
		...
		if ($query = mysql_query("SELECT * FROM `wed_news_items2` WHERE `id` = '$id'")){
			...
			if ($item = mysql_fetch_array($query)){
				...
				$sql = "UPDATE `wed_news_items2` SET `is_favorite` = '$is_favorite' WHERE `id` = '$id'";
				...
				@header("Location: /wedadmin/news_items2.php");
	...
	if (isset($_GET["deletei"])){
		...
		mysql_query("DELETE FROM `wed_news_items2` WHERE `id` = '$id'");
		...
		if (count($date_a)==3){
			...
			if ($_FILES['small_picture']['name']) {
				...
				if ($small_uploaded){
					...
					mysql_query("UPDATE `wed_news_items2` SET `small_picture` = '$to' WHERE `id` = '$id'");
				...
				mysql_query("UPDATE `wed_news_items2` SET `date` = '$date', `title` = '$title', `short_text` = '$short_text',`full_text` = '$full_text' WHERE `id` = '$id'");
	...
	if (isset($_GET["createi"])){
		...
		if (checkdate($_POST["date_m"],$_POST["date_d"],$_POST["date_y"])){
			...
			mysql_query("INSERT INTO `wed_news_items2` (`date`, `title`, `short_text`, `full_text`, `small_picture`) VALUES ('$date', '$title', '$short_text', '$full_text', '$name')");
			...
			if ($small_uploaded){
				...
				mysql_query("UPDATE `wed_news_items2` SET `small_picture` = '$new_filename' WHERE `id` = '$id'");
	...
	if (isset($_GET["editi"])){
		...
		if ($item = mysql_fetch_array(mysql_query("SELECT * FROM `wed_news_items2` WHERE `id` = '$id'"))){
			...
			$xihna_enabled = true;	
                ...
                echo '<FORM method="post" enctype="multipart/form-data" action="'.WA_URL.'news_items2.php?updatei='.$item["id"].'">';
			...
			echo '[ <A href="/wedadmin/news_items2.php?delete_picture=1&id='.$item["id"].'" onClick="javascript: if (confirm('."'Удалить картинку?')) { return true;} else { return false;}\"".'>удалить</A> ]<BR/>';
				...
				echo '<a href="#ab" onclick="open(\'/wedadmin/news_images2.php?id='.$item['id'].'\', \'popUpWin\', \'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,copyhistory=yes,width=600,height=700,left=0, top=0,screenX=0,screenY=0\');">Загрузить изображения</a><br><br>';
	...
	if (isset($_GET["addi"])){
		...
		$xihna_enabled = true;
                ...
                echo '<div style="margin:25px;"><FORM method="post" enctype="multipart/form-data" action="'.WA_URL.'news_items2.php?createi">';
				...
				echo '<a href="#ab" onclick="open(\'/wedadmin/news_images2.php?id='.$nextid.'\', \'popUpWin\', \'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,copyhistory=yes,width=600,height=700,left=0, top=0,screenX=0,screenY=0\');">Загрузить изображения</a><br><br>';
	...
	$news_total = mysql_result(mysql_query("SELECT COUNT(*) FROM `wed_news_items2`"), 0);
    ...
    echo '<center>[ <A href="'.WA_URL.'news_items2.php?addi">добавить новость</A> ]</center><BR/>';
	...
	if ($pages_count > 1){
		...
		while ($i < ($pages_count)){
			...
			echo ' <A href="'.WA_URL.'news_items2.php?page='.$i.'">';
	...
	$query = mysql_query("SELECT * FROM `wed_news_items2` ORDER BY `is_favorite` DESC, `date` DESC, `id` DESC LIMIT $from, $count");
		...
		if (mysql_num_rows($query)){
			...
			while ($item = mysql_fetch_array($query)) {
				...
				echo ' <A href="/wedadmin/news_items2.php?fav='.$item["id"].'"><img src="/wedadmin/images/star_';
				...
				echo '<a href="'.WA_URL.'news_items2.php?editi='.$item["id"].'">';
				...
				echo '<a href="'.WA_URL.'news_items2.php?deletei='.$item["id"].'" onClick="javascript: if (confirm(\'Удалить новость?\')) { return true;} else { return false;}">';
		...
		//		echo '<TR><TD>'.date('d.m.y',$item["date"]).'</TD><TD>'.$item["title"].'</TD><TD>[ <A href="'.WA_URL.'news_items2.php?edit='.$item["id"].'">изменить</A> ]</TD></TR>';
		...
		echo '<BR/><center>[ <A href="'.WA_URL.'news_items2.php?addi">добавить новость</A> ]</center><BR/><BR/>';
