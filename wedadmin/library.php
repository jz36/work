...
print <<<HTML
<P><a href="/library.php?id=$id" target="_blank">посмотреть на сайте</a></P>
HTML;
	if ($item = mysql_fetch_array(mysql_query("SELECT * FROM `wed_library_items` WHERE `id` = '$id'"))){
		...
		echo ' Заголовок:<BR/><INPUT type="text" name="title" value="'.htmlspecialchars($item["title"]).'" size="70"><BR/><BR/>';
		echo ' TITLE:<BR/><TEXTAREA class="w90" id="meta_description" name="meta_description" rows=3 cols=50>'.$item["meta_description"].'</TEXTAREA><BR/><BR/>';
