...
function resize_image_oneparam($filename, $new_filename, $maxwidth, $maxheight){
	...
	$c_url = '/news2/customimages/';

	$c_path = NX_PATH.'news2/customimages/';
	...
	if (isset($_GET["id"])){
		...
		while ($i < ($pages_count)){
			...
			echo ' <A href="/wedadmin/news_images2.php?id='.$id.'&page='.$i.'">';
		...
		foreach ($files as $filename){
			...
			if ($imginfo){
				...
				echo '<form action="/wedadmin/news_images2.php?id='.$id.'&resize" method="post">';
			...
			echo '</TD><TD><A href="/wedadmin/news_images2.php?id='.$id.'&delete='.base64_encode($filename).'" onClick="javascript: if (confirm(\'Удалить?\')) { return true;} else { return false;}"><img src="/wedadmin/images/b_drop.png" border="0" /></A></TD></TR>';

		}

		echo '</TABLE>';

		echo '<BR/><FORM method="post" enctype="multipart/form-data" action="/wedadmin/news_images2.php?id='.$id.'&upload"><INPUT type="file" name="picture"><BR/><BR/><INPUT type="submit" value="добавить"></FORM><BR/><BR/>';
