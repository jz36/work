...
if ($id=="add") // сюда попадаем когда добавляем новый пункт edit класса
{

if ($cookie==0)
	{

	del_temp_file();	// удаляем temp* файл
	$tempfile=SITE_FILES_TMP_DIR."/temp".rand(0,32000).rand(0,32000).".html";
	$file=fopen($tempfile, "w");			// создаем новый temp* файл
	fclose($file);
	}
else
	{
	$name=$this->name;
	$parts=file($tempfile);					// читаем файл
	$cont="";
	for ($i=0;$i<count($parts);$i++)	// тут просто переносим данные из temp*  в переменную $cont
		$cont.=$parts[$i]."\n";			//
	del_temp_file();
	$tempfile=SITE_FILES_TMP_DIR."/temp".rand(0,32000).rand(0,32000).".html";
	$file=fopen($tempfile, "w");
	fputs($file, lec_image($cont));	// создаем новый файл temp* и переносим туда данные, вставляя для картинок
	fclose($file);						//  переменную $rand чтобы обмануть кэш
	}
}
else {	// тут если раздел не добавляется, а модифицируется имеющийся

	if ($cookie==0)	// раздел модифицируется и cookie=0
		{
		...
		del_temp_file();						//  удаляем старый temp* файл
		$tempfile=SITE_FILES_TMP_DIR."/temp".rand(0,32000).rand(0,32000).".html";
		$file=fopen($tempfile, "w");		//  создаем новый файл и пишем в него стандартный хедер
		fputs($file, "<html><META HTTP-EQUIV='PRAGMA' CONTENT='NO-CACHE'><META http-equiv=Content-Type content=\"text/html; charset=windows-1251\"><link href='".PATH."style.css' rel='stylesheet' type='text/css'><BODY text='#333333' style='padding:10px;background-color:#fff;".@ADMIN_ACTIVEX_BG_STYLE."' id='content' class='edit_content'>");
		$row[1]=$this->prepare_content($row[1]);  // ??? вызываем метод prepare_content с параметром content
																// считанным из таблицы
		fputs($file, lec_image($row[1]));			// модим для обмана кэша
		fputs($file, "</BODY></html>");			// допишем окончание html и закроем файл
		fclose($file);									// ТЕПЕРЬ В TEMP* ФАЙЛЕ ОБРАБОТАННЫЕ ДАННЫЕ ИЗ CONTENT !!!
		}
	else	// раздел модифицируется и cookie<>0
		{

		$name=$this->name;				// значение поля name из таблицы
		$parts=file($tempfile);			// считываем подготовленный ранее файл с контентом
		$cont="";
		for ($i=0;$i<count($parts);$i++)
			$cont.=$parts[$i]."\n";	// записываем контент в $cont
		del_temp_file();					// удаляем файл
		$tempfile=SITE_FILES_TMP_DIR."/temp".rand(0,32000).rand(0,32000).".html";
		$file=fopen($tempfile, "w");
		fputs($file, lec_image($cont)); // создаем новый ТЕМР файл и пишем туда модиф. контент для кэша
		fclose($file);
		}
	}
...
<IMG alt="Жирный" onclick="doit('BOLD');" onMouseOver="onImg(this);" onMouseOut="ofImg(this);" src="<?=SITE_ADMIN_DIR?>/img/activex/text_bold2.gif" height=24 width=24 border=0>
<IMG alt="Курсив" onclick="doit('ITALIC');" onMouseOver="onImg(this);" onMouseOut="ofImg(this);" height=24 src="<?=SITE_ADMIN_DIR?>/img/activex/text_italic2.gif" width=24 border=0>
<IMG alt="Подчеркивание" onclick="doit('UNDERLINE');" onMouseOver="onImg(this);" onMouseOut="ofImg(this);" height=24 src="<?=SITE_ADMIN_DIR?>/img/activex/text_underline2.gif" width=24 border=0>
<IMG 						height=24 src="<?=SITE_ADMIN_DIR?>/img/activex/div.gif" width=9 border=0>
<IMG alt="По левому краю" onclick="doit('JUSTIFYLEFT');" onMouseOver="onImg(this);" onMouseOut="ofImg(this);" src="<?=SITE_ADMIN_DIR?>/img/activex/text_align_left.gif" border=0>
<IMG alt="По ширине" onclick="AlJ();" onMouseOver="onImg(this);" onMouseOut="ofImg(this);" src="<?=SITE_ADMIN_DIR?>/img/activex/text_align_justify.gif" height=24 width=24 border=0>
<IMG alt="По центру" onclick="doit('JUSTIFYCENTER');" onMouseOver="onImg(this);" onMouseOut="ofImg(this);" src="<?=SITE_ADMIN_DIR?>/img/activex/text_align_center.gif" height=24 width=24 border=0>
<IMG alt="По правому краю" onclick="doit('JUSTIFYRIGHT');" onMouseOver="onImg(this);" onMouseOut="ofImg(this);" height=24 src="<?=SITE_ADMIN_DIR?>/img/activex/text_align_right.gif" width=24 border=0>
<IMG 						height=24 src="<?=SITE_ADMIN_DIR?>/img/activex/div.gif" width=9 border=0>
<IMG alt="Нумерованый список" onclick="doit('ORDERLIST');" onMouseOver="onImg(this);" onMouseOut="ofImg(this);" height=24 src="<?=SITE_ADMIN_DIR?>/img/activex/num_ul.gif" width=24 border=0>
<IMG alt="Ненумерованный список" onclick="doit('UNORDERLIST');" onMouseOver="onImg(this);" onMouseOut="ofImg(this);" height=24 src="<?=SITE_ADMIN_DIR?>/img/activex/ul.gif" width=24 border=0>
<IMG 						height=24 src="<?=SITE_ADMIN_DIR?>/img/activex/div.gif" width=9 border=0>
<IMG alt="Гиперссылка" onclick="Link();" onMouseOver="onImg(this);" onMouseOut="ofImg(this);" height=24 src="<?=SITE_ADMIN_DIR?>/img/activex/url.gif" width=24 border=0>
<IMG alt="Убрать гиперссылку" onclick="UnLink();" onMouseOver="onImg(this);" onMouseOut="ofImg(this);" height=24 src="<?=SITE_ADMIN_DIR?>/img/activex/nolink.gif" width=24 border=0>
<IMG 						height=24 src="<?=SITE_ADMIN_DIR?>/img/activex/div.gif" width=9 border=0>
<IMG alt="Якорь" onclick="crAnchor();" onMouseOver="onImg(this);" onMouseOut="ofImg(this);" height=24 src="<?=SITE_ADMIN_DIR?>/img/activex/anchor_cr.gif" width=24 border=0>
<IMG alt="Убрать якорь" onclick="unAnchor();" onMouseOver="onImg(this);" onMouseOut="ofImg(this);" height=24 src="<?=SITE_ADMIN_DIR?>/img/activex/anchor_un.gif" width=24 border=0>
<IMG 						height=24 src="<?=SITE_ADMIN_DIR?>/img/activex/div.gif" width=9 border=0>
<IMG alt="Вставить из буфера" onclick="doit('PASTE');" onMouseOver="onImg(this);" onMouseOut="ofImg(this);" src="<?=SITE_ADMIN_DIR?>/img/activex/paste.gif" height=24 width=24 border=0>
<IMG alt="Копировать в буфер" onclick="doit('COPY');" onMouseOver="onImg(this);" onMouseOut="ofImg(this);" src="<?=SITE_ADMIN_DIR?>/img/activex/copy.gif" height=24 width=24 border=0>
<IMG alt="Вырезать" onclick="doit('CUT');" onMouseOver="onImg(this);" onMouseOut="ofImg(this);" src="<?=SITE_ADMIN_DIR?>/img/activex/cut.gif" height=24 width=24 border=0>
<IMG alt="Вставить картинку" onclick="doit('IMAGE');" onMouseOver="onImg(this);" onMouseOut="ofImg(this);" height=24 src="<?=SITE_ADMIN_DIR?>/img/activex/insert_image2.gif" width=24 border=0>
<IMG alt="Вставить символ" onclick="InsChar();" onMouseOver="onImg(this);" onMouseOut="ofImg(this);" src="<?=SITE_ADMIN_DIR?>/img/activex/symbol.gif" height=24 width=24 border=0>
</TD></TR>

<tr bgcolor="#F0F0F0" height="26"><td nowrap>
<img alt="Вставить таблицу" src="<?=SITE_ADMIN_DIR?>/img/activex/insert-table.gif" width="24" height="24" onclick='InsTable()' onMouseOver="onImg(this);" onMouseOut="ofImg(this);">
<img alt="Вставить столбец" src="<?=SITE_ADMIN_DIR?>/img/activex/insert_col.gif" width="24" height="24" onclick='InsCol()' onMouseOver="onImg(this);" onMouseOut="ofImg(this);">
<img alt="Вставить строку" src="<?=SITE_ADMIN_DIR?>/img/activex/insert_row.gif" width="24" height="24" onclick='InsRow()' onMouseOver="onImg(this);" onMouseOut="ofImg(this);">
<img alt="Слить ячейки" src="<?=SITE_ADMIN_DIR?>/img/activex/merge_cells.gif" width="24" height="24" onclick='MargeCells()' onMouseOver="onImg(this);" onMouseOut="ofImg(this);">
<img alt="Разбить ячейки" src="<?=SITE_ADMIN_DIR?>/img/activex/split_cells.gif" width="24" height="24" onclick='SplitCells()' onMouseOver="onImg(this);" onMouseOut="ofImg(this);">
<img alt="Удалить ряд" src="<?=SITE_ADMIN_DIR?>/img/activex/del_row.gif" width="24" height="24" onclick='DelRows()' onMouseOver="onImg(this);" onMouseOut="ofImg(this);">
<img alt="Удалить колонку" src="<?=SITE_ADMIN_DIR?>/img/activex/del_col.gif" width="24" height="24" onclick='DelCols()' onMouseOver="onImg(this);" onMouseOut="ofImg(this);">
<img alt="Удалить ячейки" src="<?=SITE_ADMIN_DIR?>/img/activex/del_cell.gif" width="24" height="24" onclick='DelCells()' onMouseOver="onImg(this);" onMouseOut="ofImg(this);">
<IMG alt="Заменить ячейку на заголовок таблицы / и наоборот"  src="<?=SITE_ADMIN_DIR?>/img/activex/th.gif" onclick='InsTH()' onMouseOver="onImg(this);" onMouseOut="ofImg(this);" height=24 border=0>
<IMG alt="Вставить разделитель" onclick='InsHR()' onMouseOver="onImg(this);" onMouseOut="ofImg(this);" height=24 src="<?=SITE_ADMIN_DIR?>/img/activex/hr.gif" border=0>
<img 						height=24 src="<?=SITE_ADMIN_DIR?>/img/activex/div.gif" width=9 border=0>
<img alt="Параграф текста" src="<?=SITE_ADMIN_DIR?>/img/activex/p.gif" width="24" height="24" onclick='InsP();' onMouseOver="onImg(this);" onMouseOut="ofImg(this);">
<img alt="Заголовок большой" src="<?=SITE_ADMIN_DIR?>/img/activex/h1.gif" width="24" height="24" onclick='InsH1();' onMouseOver="onImg(this);" onMouseOut="ofImg(this);">
<img alt="Заголовок средний" src="<?=SITE_ADMIN_DIR?>/img/activex/h2.gif" width="24" height="24" onclick='InsH2();' onMouseOver="onImg(this);" onMouseOut="ofImg(this);">
<img alt="Заголовок обычный" src="<?=SITE_ADMIN_DIR?>/img/activex/h3.gif" width="24" height="24" onclick='InsH3();' onMouseOver="onImg(this);" onMouseOut="ofImg(this);">
<img alt="Заголовок маленький" src="<?=SITE_ADMIN_DIR?>/img/activex/h3.gif" width="24" height="24" onclick='InsH4();' onMouseOver="onImg(this);" onMouseOut="ofImg(this);">
<img 						height=24 src="<?=SITE_ADMIN_DIR?>/img/activex/div.gif" width=9 border=0>
<img alt="Нижний индекс" src="<?=SITE_ADMIN_DIR?>/img/activex/sub.gif" width="24" height="24" onclick='InsSub();' onMouseOver="onImg(this);" onMouseOut="ofImg(this);">
<img alt="Верхний индекс" src="<?=SITE_ADMIN_DIR?>/img/activex/sup.gif" width="24" height="24" onclick='InsSup();' onMouseOver="onImg(this);" onMouseOut="ofImg(this);">
<img alt="Убрать теги" src="<?=SITE_ADMIN_DIR?>/img/activex/nohtml.gif" width="24" height="24" onclick='KillHTML();' onMouseOver="onImg(this);" onMouseOut="ofImg(this);">
<img alt="Убрать форматирование" src="<?=SITE_ADMIN_DIR?>/img/activex/nofont.gif" width="24" height="24" onclick='DelFormat();' onMouseOver="onImg(this);" onMouseOut="ofImg(this);">
<img alt="Убрать форматирование" src="<?=SITE_ADMIN_DIR?>/img/activex/nofont.gif" width="24" height="24" onclick='SpellCheck();' onMouseOver="onImg(this);" onMouseOut="ofImg(this);">
</td></tr>

<TR bgColor="#F1F6FF" height="37"><TD valign=bottom>
<?	if (main_access()==3 || (main_access()==2 && $id=="add")){?>
<IMG  height=23 alt="Сохранить изменения" src="<?=SITE_ADMIN_DIR?>/img/activex/save.gif"  width=91 border=0
	onMouseDown="ShowText();" 
	onClick="javascript: if (data.name.value=='') alert('Заполните поле с названием!'); else ready();"
	onMouseOver="onImg(this);"
	onMouseOut="ofImg(this);">
<?}?>
<IMG height=23 alt="Отменить все внесенные изменения" src="<?=SITE_ADMIN_DIR?>/img/activex/un_edit.gif" width=89 border=0
	onClick="javascript:arr=new Array();arr=location.href.split('<?=PAGE?>');path=arr[0]; init(path, '<? echo $tempfile ?>');" 
	onMouseOver="onImg(this);" 
	onMouseOut="ofImg(this);">
<IMG 						height=24 src="<?=SITE_ADMIN_DIR?>/img/activex/div.gif" width=9 border=0>
<IMG alt="Отменить предыдущее действие" onclick="doit('UNDO');" onMouseOver="onImg(this);" onMouseOut="ofImg(this);" src="<?=SITE_ADMIN_DIR?>/img/activex/undo2.gif" height=24 width=24 border=0>
<IMG alt="Применить предыдущее действие" onclick="doit('REDO');" onMouseOver="onImg(this);" onMouseOut="ofImg(this);" src="<?=SITE_ADMIN_DIR?>/img/activex/redo2.gif" height=24 width=24 border=0>
<img 						height=24 src="<?=SITE_ADMIN_DIR?>/img/activex/div.gif" width=9 border=0>
<img alt="Показать в виде текста" src="<?=SITE_ADMIN_DIR?>/img/activex/view-text.gif" width="24" height="24" onclick='ShowText();' onMouseOver="onImg(this);" onMouseOut="ofImg(this);">
<img alt="Показать в виде HTML" src="<?=SITE_ADMIN_DIR?>/img/activex/view-html.gif" width="24" height="24" onclick='ShowHtml();' onMouseOver="onImg(this);" onMouseOut="ofImg(this);">
<INPUT type=hidden name=htmlcode><INPUT type=hidden value="<? echo $main ?>" name=main><input type=hidden name=adcookie value=1>
<? if ($id=="add") echo "<input type=hidden name=add value=1>"; else echo "<input type=hidden name=id value=".$id.">";?>
</FORM></TD></TR>
<TR><td><OBJECT
	id=HTMLEdit 
	classid=CLSID:2D360201-FFF5-11d1-8D03-00A0C959BC0A
	class=small 
	width=100% height=350 bgcolor="#FFFFFF"><PARAM NAME="ShowBorders" VALUE="-1"></OBJECT>
...
<OBJECT CLASSID="clsid:47B0DFC7-B7A3-11D1-ADC5-006008A5848c" ID="TableObj" STYLE="visibility:hidden">
	<PARAM NAME=NumRows VALUE=3>
	<PARAM NAME=NumCols VALUE=3>
	<PARAM NAME=Caption VALUE="">
	<PARAM NAME=TableAttrs VALUE="">
	<PARAM NAME=CellAttrs VALUE=""></OBJECT></TD>
</TR></TABLE>
<SCRIPT language=JScript src="<?=SITE_ADMIN_DIR?>/js/editconst.js"></SCRIPT>
<SCRIPT language=JScript src="<?=SITE_ADMIN_DIR?>/js/htmledit.js"></SCRIPT>


<SCRIPT LANGUAGE=JavaScript FOR=HTMLEdit EVENT=ShowContextMenu>
//===================================================================
  menuStrings = new Array();
  menuStates = new Array();

  menuStrings[0] = "Копировать";
  menuStrings[1] = "Вставить";
  menuStrings[2] = "Вырезать";
  menuStrings[3] = "Выделить все";
  menuStrings[4] = "";
  menuStrings[5] = "Поиск в тексте";
  ...
  menuStates[5] = OLE_TRISTATE_UNCHECKED;

  HTMLEdit.SetContextMenu( menuStrings, menuStates );
</SCRIPT>

<SCRIPT LANGUAGE=JavaScript FOR=HTMLEdit EVENT=ContextMenuAction(itemIndex)>
  //alert( "You selected item " + itemIndex );
  if(itemIndex==0) {
	HTMLEdit.ExecCommand(DECMD_COPY,OLECMDEXECOPT_DODEFAULT,0);
	}
  if(itemIndex==1) {
	HTMLEdit.ExecCommand(DECMD_PASTE,OLECMDEXECOPT_DODEFAULT,0);
	}
	if(itemIndex==2) {
	HTMLEdit.ExecCommand(DECMD_CUT,OLECMDEXECOPT_DODEFAULT,0);
	}
	if(itemIndex==3) {
	HTMLEdit.ExecCommand(DECMD_SELECTALL,OLECMDEXECOPT_DODEFAULT,0);
	}
	if(itemIndex==5) {
...
<script>  
//alert (HTMLEdit.styleSheets);

	arr=new Array();
	arr=location.href.split('<?=PAGE?>');
	path=arr[0];
	init(path, '<? echo $tempfile ?>');
</script>
<?
...
global $rand;global $sub;global $main; global $HTTP_POST_VARS;
$tempmain=$main;
$tempfile=show_temp_file();
$admin_preview=1;
//echo($COOCIE["inputs"]);
//print_r($GLOBAL)
#Загружаем index.php в режиме предпросмотра, за это отвечает файл content/admin_preview.php
require(SPAGE); 
exit;
}
...
for ($i=0;$i<count($parts);$i++)
	{
	$dopparts=split(">", $parts[$i]);
	if (count($dopparts)<2)
		$newstr.=$parts[$i];
	else
		{
		$dopparts[0]=del_single_space($dopparts[0]);
		$tegparts=split(" ", $dopparts[0]);
		$razr=0;
		if ($tegparts[0]=="P")
			{
			$newstr.="<P";
			for ($j=0;$j<count($tegparts);$j++)
				{
					if (eregi("align=", $tegparts[$j]))
						$newstr.=" ".$tegparts[$j];
					}
			$newstr.=">";
			}
		elseif ($tegparts[0]=="A")
			...
			for ($j=0;$j<count($tegparts);$j++)
				{
					if (ereg("href", $tegparts[$j]))
						$newstr.=" ".$tegparts[$j];
					if (ereg("name", $tegparts[$j]))
						$newstr.=" ".$tegparts[$j];
				}
			$newstr.=">";
			}
		elseif ($tegparts[0]=="TR")
			{
			for ($j=0;$j<count($tegparts);$j++)
				{
					if (eregi("name=name", $tegparts[$j])) $newstr.="<TR>";
					}
			}
		elseif ($tegparts[0]=="TD")
			{
			for ($j=0;$j<count($tegparts);$j++)
				{
					if (ereg("name=name", $tegparts[$j]))
						{
						$newstr.="<TD";
						$razr=1;
						}
			...
			if ($razr>0)
				{
					for ($j=0;$j<count($tegparts);$j++)
						...
						if (eregi("align=", $tegparts[$j]))
							$newstr.=" ".$tegparts[$j];
					if (eregi("width=", $tegparts[$j]))
						$newstr.=" ".$tegparts[$j];
					if (eregi("colspan=", $tegparts[$j]) || eregi("rowspan=", $tegparts[$j]))
						$newstr.=" ".$tegparts[$j];
						}
					$newstr.=">";
					}
